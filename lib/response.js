...
var _response = {
  ...
  set status(code) {
    ...
    if (this.request.httpVersionMajor < 2) this.res.statusMessage = _statuses.default[code];
    if (this.body && _statuses.default.empty[code]) this.body = null;
  },

  get message() {
    return this.res.statusMessage || _statuses.default[this.status];
  },
  ...
  set body(val) {
    ...
    if (val == null) {
      if (!_statuses.default.empty[this.status]) this.status = 204; // 204 means no content
    ...
    if (typeof val.pipe === 'function') {
      (0, _onFinished.default)(this.res, _destroy.default.bind(null, val));
  ...
  get writable() {
    ...
    var socket = this.res.socket; // There are already pending outgoing response, but still writable
